...
import { EMAIL_QUEUE_NAME, EmailJobData, emailQueue, registerLocalMockWorkerHandler } from './email.queue.js';
...
export async function processEmailJob(job: { id?: string; data: EmailJobData; attemptsMade: number; opts: { attempts: number } }) {
  ...
  try {
    ...
    logger.error(
      { emailId, error: sendError.message },
...
export function createEmailWorker(): any {
  if (isMock) {
    logger.info('[Local Memory Mode] Integrated Worker registered with in-memory dispatcher');
    registerLocalMockWorkerHandler(processEmailJob);
    return {
      close: async () => {},
    };
  }
  ...
  const worker = new Worker<EmailJobData>(
    ...
    async (job: Job<EmailJobData>) => {
      return processEmailJob(job as any);
    },
    {
  ...
  worker.on('error', (err) => {
    logger.warn('Worker error connecting to Redis: ' + err.message);
  });
